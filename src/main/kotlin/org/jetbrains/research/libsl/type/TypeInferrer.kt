...
class TypeInferrer(private val context: LslContextBase) {
    ...
    fun getExpressionType(expression: Expression): Type {
        return when (expression) {
            ...
            is LeftUnaryOpExpression -> getExpressionType(expression.value)
            is RightUnaryOpExpression -> getExpressionType(expression.value)
    ...
    private fun getQualifiedAccessType(access: QualifiedAccess): Type {
        return when (access) {
            ...
            // TODO (Refactor MAYBE?)
            is ThisAndParentAccess -> getQualifiedAccessType(access.lastChild)
        }
    }
